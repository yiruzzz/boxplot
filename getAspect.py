...
def getList(path, datas):
    ...
    for i in range(len(annotations)):
        ...
        # 得到一个各类面积的字典
        aspect = dict_temp["bbox"][3] / dict_temp["bbox"][2]
        dict_13[str(dict_temp["category_id"])].append(aspect)
...
# bplot = plt.boxplot(list, labels = labels)
bplot = plt.boxplot(list, labels=labels, showfliers=False, widths=0.6, capprops={'color':'g', 'linewidth':2}, medianprops={'color':'DarkBlue'})
plt.show()



# print(df.shape)
# dataf.to_excel("C:/Users/zyr/Desktop/aspect.xlsx", index=False)